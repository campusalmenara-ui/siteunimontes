...
const SLIDE_INTERVAL_MS = 3 * 60_000;
...
const REDE_CONFIG: Record<string, { label: string; iconBg: string; btnBg: string; btnLabel: string }> = {
  instagram: { label: 'Instagram', iconBg: 'linear-gradient(135deg,#833ab4,#fd1d1d,#fcb045)', btnBg: 'linear-gradient(135deg,#833ab4,#fd1d1d,#fcb045)', btnLabel: 'Ver no Instagram' },
  youtube:   { label: 'YouTube',   iconBg: '#ff0000', btnBg: '#ff0000', btnLabel: 'Ver no YouTube' },
};
...
async function fetchCSV(gid?: string): Promise<string[][]> {
  ...
  let res = await fetch(url), text = await res.text();
            ...
            return (
              <div key={idx} className={`flex-1 min-h-0 rounded-xl px-4 py-2 flex gap-3 items-center overflow-hidden ${isNoClass ? 'bg-white/5 opacity-60' : 'bg-white/10'}`}>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {!isNoClass && c.observation && <p className="text-yellow-300/80 text-[10px] truncate">⚠ {c.observation}</p>}
                </div>
              </div>
            );
          })}
...
// ─── Notícias + Redes (carrossel normal) ───────────────────────────────────────
...
function NewsPanel({ feed }: { feed: FeedItem[] }) {
  ...
  if (feed.length === 0) return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-2 mb-4"><div className="w-1 h-6 bg-yellow-400 rounded-full flex-shrink-0" /><h2 className="text-xl font-bold text-white">Notícias & Redes</h2></div>
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      <div className="flex items-center gap-2 mb-3 flex-shrink-0">
        ...
        {totalPages > 1 && <span className="ml-auto text-blue-300 text-xs">{page + 1}/{totalPages}</span>}
      </div>
      <div className="flex-1 flex flex-col gap-3 min-h-0 justify-center">
        {slice.map((item, i) => {
          ...
          return (
            <div key={`${page}-${i}`} className="rounded-xl overflow-hidden relative flex-shrink-0"
              ...
              <div className="absolute top-2 left-2 z-10">
                {isSocial ? (
                  <div className="w-6 h-6 rounded-full flex items-center justify-center text-white shadow" style={{ background: cfg!.iconBg }}>
                    {rede === 'youtube' ? <YoutubeIcon size={12} /> : <InstagramIcon size={12} />}
                  </div>
                ) : (item.kind === 'news' && item.categoria) ? (
                  <span className="bg-yellow-400 text-blue-900 text-[9px] font-bold px-2 py-0.5 rounded-full">{item.categoria}</span>
                ) : null}
              </div>
...
function NewsSlide({ feed, onDone }: { feed: FeedItem[]; onDone: () => void }) {
  ...
  const titulo   = isSocial ? item.titulo : '';
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col" style={{ background: '#0f1f4a' }}>
      ...
      <div className="absolute inset-0 overflow-hidden">
        <img src={img} alt="" className="w-full h-full object-cover opacity-15 blur-sm"
          onError={e => { (e.target as HTMLImageElement).style.display='none'; }} />
        ...
        <div className="flex flex-col gap-4 flex-shrink-0" style={{ width: '38%' }}>
          ...
          <div>
            {!isSocial && item.categoria && (
              <span className="inline-block bg-yellow-400 text-blue-900 text-sm font-bold px-4 py-1 rounded-full mb-2">{item.categoria}</span>
            )}
            {!isSocial && (
        ...
        <div className="flex-1 flex flex-col justify-center min-h-0 min-w-0">
          <h1 className="text-white font-bold leading-tight mb-4 drop-shadow-lg"
            style={{ fontSize: isSocial
              ? (titulo.length > 100 ? '1.4rem' : titulo.length > 50 ? '1.8rem' : '2.2rem')
              : (item.title.length > 80 ? '1.6rem' : item.title.length > 50 ? '2rem' : '2.4rem') }}>
            {isSocial ? titulo : item.title}
          </h1>
          {!isSocial && hasText && (
            <p className="text-blue-100 leading-relaxed overflow-hidden"
              style={{
                ...
                display: '-webkit-box', WebkitLineClamp: item.texto.length > 600 ? 12 : 10, WebkitBoxOrient: 'vertical',
              }}>
              {item.texto}
            </p>
          )}
          ...
          {isSocial && legenda && (
            <p className="text-blue-100 leading-relaxed overflow-hidden"
              style={{ fontSize: legenda.length > 400 ? '0.82rem' : '0.92rem', display: '-webkit-box', WebkitLineClamp: 12, WebkitBoxOrient: 'vertical' }}>
              {legenda}
            </p>
          )}
        </div>
...
export default function TvDisplay() {
  ...
  const loadWeather = useCallback(async () => {
    try {
      const res  = await fetch('https://api.open-meteo.com/v1/forecast?latitude=-16.18&longitude=-40.69&current=temperature_2m,weathercode,windspeed_10m&timezone=America%2FSao_Paulo');
      const json = await res.json();
      setWeather({ temp: json.current.temperature_2m, code: json.current.weathercode, wind: json.current.windspeed_10m });
  ...
  const loadAll = useCallback(async () => {
    try {
      ...
      for (let i = 1; i < agendaRows.length; i++) {
        ...
        const curso = cells[2]?.trim() || '', periodo = cells[3]?.trim() || '', materia = cells[4]?.trim() || '';
        const professor = cells[5]?.trim() || '', hours = cells[6]?.trim() || '', obs = cells[7]?.trim() || '';
        if (!curso || !periodo || !materia) continue;
        loadedClasses.push({
          period: `${periodo} PERÍODO`, subject: materia,
      ...
      for (const cells of newsRows.slice(1)) {
        const imageUrl = cells[0]?.trim() || '', title = cells[1]?.trim() || '';
        const categoria = cells[3]?.trim() || '', parsed = parseDateBR(cells[4]?.trim() || '');
      ...
      for (const cells of calRows.slice(1)) {
        const dia = parseInt(cells[0]?.trim() || '', 10), mes = parseMes(cells[1]?.trim() || '');
        const ano = parseInt(cells[2]?.trim() || '', 10), atividade = cells[3]?.trim() || '';
  ...
  useEffect(() => {
    const id = setInterval(() => { if (feed.length > 0) setSlideMode(true); }, SLIDE_INTERVAL_MS);
    return () => clearInterval(id);
  }, [feed.length]);
  ...
  return (
    <>
      ...
      <div className="h-screen overflow-hidden flex flex-col select-none relative"
        ...
        <div className="absolute inset-0 overflow-hidden">
          <div style={{ position:'absolute', width:'60%', height:'60%', borderRadius:'50%', filter:'blur(80px)', opacity:0.25, background:'#2563eb', animation:'blob1 18s ease-in-out infinite' }} />
          <div style={{ position:'absolute', width:'50%', height:'50%', borderRadius:'50%', filter:'blur(80px)', opacity:0.2,  background:'#1d4ed8', animation:'blob2 22s ease-in-out infinite' }} />
          <div style={{ position:'absolute', width:'40%', height:'40%', borderRadius:'50%', filter:'blur(60px)', opacity:0.15, background:'#3b82f6', animation:'blob3 16s ease-in-out infinite' }} />
        </div>
        <style>{`
          @keyframes meshGradient { 0%{background-position:0% 50%} 50%{background-position:100% 50%} 100%{background-position:0% 50%} }
          @keyframes blob1 { 0%,100%{top:-10%;left:-10%} 33%{top:30%;left:50%} 66%{top:60%;left:10%} }
          @keyframes blob2 { 0%,100%{top:60%;right:-10%;left:auto} 33%{top:10%;right:30%;left:auto} 66%{top:50%;right:60%;left:auto} }
          @keyframes blob3 { 0%,100%{bottom:-10%;left:30%} 33%{bottom:50%;left:60%} 66%{bottom:20%;left:10%} }
        `}</style>

        <header className="relative z-10 flex-shrink-0 px-8 py-3 flex items-center justify-between border-b border-white/10">
          <div className="flex-1 flex items-center gap-3"><WeatherWidget data={weather} /></div>
          <div className="flex justify-center">
            <img src="https://d2xsxph8kpxj0f.cloudfront.net/310419663030187894/57Ypr7wbFX6eHCZZ7V6o8w/logo_cc3239cb.png"
              alt="Unimontes Campus Almenara" className="h-12 w-auto brightness-0 invert" />
          </div>
          <div className="flex-1 flex justify-end"><Clock /></div>
        </header>
        ...
        <footer className="relative z-10 flex-shrink-0 px-8 py-2 border-t border-white/10 flex items-center justify-between">
          ...
          {lastUpdated && <p className="text-blue-100 text-xs">Atualizado às {pad(lastUpdated.getHours())}:{pad(lastUpdated.getMinutes())}</p>}
        </footer>
      </div>
    </>
  );

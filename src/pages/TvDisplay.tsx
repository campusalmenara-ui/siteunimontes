        ...
        <div className="flex-1 flex flex-col justify-center min-h-0 min-w-0">
          <h1 className="text-white font-bold leading-tight mb-4 drop-shadow-lg"
            style={{ fontSize: isSocial
              ? (titulo.length > 80 ? '1.6rem' : titulo.length > 50 ? '2rem' : '2.4rem')
          ...
          {/* Social: legenda completa (coluna B) como texto do corpo */}
          {isSocial && legenda && (
            <p className="text-blue-100 leading-relaxed overflow-hidden"
              style={{
                fontSize: legenda.length > 600 ? '0.85rem' : legenda.length > 300 ? '0.9rem' : '0.95rem',
                display: '-webkit-box',
                WebkitLineClamp: legenda.length > 600 ? 12 : 10,
                WebkitBoxOrient: 'vertical',
              }}>
              {legenda}
            </p>
    ...
    try {
      ...
      const loadedSocial: (FeedItem & { kind: 'social' })[] = socialRows.slice(1)
        .map(cells => {
          ...
          const titulo  = cells[5]?.trim() || legenda.split('\n')[0] || '';
